...
const Achievement = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative w-full text-white overflow-hidden rounded-lg shadow-2xl min-h-[350px] mt-[100px]"
    >
      {/* Background Image */}
      ...
      <div className="relative z-10 py-12 md:py-16 lg:py-20 px-[20px] md:px-[30px] lg:px-[30px] xl:max-w-7xl xl:mx-auto">
        <div className="text-center achievement-content">
          ...
          {/* Description */}
          <p className="text-base sm:text-lg md:text-xl text-gray-100 max-w-4xl mx-auto mb-12 leading-relaxed font-normal">
            {descriptionText}
          </p>

          {/* Stats Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-4 md:gap-x-8 gap-y-8 sm:gap-y-10">
            {stats.map((item, i) => (
              <div
                key={i}
                className="flex flex-col items-center justify-center space-y-2 md:space-y-3 p-4"
              >
                <h3 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-[#97f03d] drop-shadow-lg">
                  <span ref={(el) => addToRefs(el, i)}>0</span>
                  {item.number > 67 && "+"}
                </h3>
                <p className="text-base sm:text-lg font-semibold text-white tracking-wide leading-tight text-center">
                  {item.label}
                </p>
              </div>
            ))}
